...
class Sofia_Componente_FAQ extends Sofia_Componente {
	...
	public function render(): string {
		...
		foreach ( array_values( $items ) as $indice => $item ) {
			...
			$html .= '<h3 class="sofia-faq__pregunta" ' . $this->atributo_editable( "items.{$indice}.pregunta" ) . '>' . $pregunta . '</h3>';
			$html .= '<p class="sofia-faq__respuesta" ' . $this->atributo_editable( "items.{$indice}.respuesta" ) . '>' . $respuesta . '</p>';
			$html .= '</div>';
		}
		$html .= '</div>';
